...
- `DashboardActivity` (`app/src/main/kotlin/org/cru/godtools/ui/dashboard/DashboardActivity.kt`) hosts a Circuit backstack rooted at `DashboardScreen`. A navigation interceptor redirects any non-dashboard `Navigator.goTo` into a **new Activity** — either `screen.startWith(activity)` for `IntentScreen`s or `startCircuitActivity(screen)` for Circuit screens — so the dashboard never pushes foreign screens onto its own backstack.
