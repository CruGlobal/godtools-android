...
6. **Open the pull request to merge it back.** GitHub shows a "Compare & pull request" prompt on your fork right after the push; otherwise use **New pull request** on the main repository and choose *compare across forks*. Set the **base repository** to `CruGlobal/godtools-android`, the **base branch** to `develop`, and the head to `<your-username>:my-feature`, then describe what the change does and why. Leave the maintainer-edits checkbox under the description box checked — it reads **Allow edits and access to secrets by maintainers**, the wording GitHub uses once a fork contains GitHub Actions workflows, as every fork of this repo does. Maintainers need it to push a snapshot commit or a small fixup onto your branch; GitHub's own caution is that it also lets them edit your fork's workflows, "which can potentially reveal values of secrets and grant access to other branches". GitHub does not offer that checkbox for forks owned by an organization, so prefer a personal fork.
...
- **A red Unit Tests check may not be your code.** For the same reason, `secrets.CODECOV_TOKEN` is empty on a fork PR, so the `tests` job's coverage upload runs untokenized. That normally still passes — Codecov supports tokenless uploads for PRs made from forks into public upstream repos, and `CruGlobal/godtools-android` is public. The step does run under `fail_ci_if_error: true` (`.github/workflows/build.yml`) though, so should the upload fail anyway (a Codecov-side outage, say) the shard goes red with no failing test. Read the job log before chasing it, and ask a maintainer if the Codecov step is what went red.
...
- uploads SARIF results to GitHub Code Scanning (the repository's Security tab) via `github/codeql-action/upload-sarif`. Note this final `upload-sarif` step is *not* guarded by `continue-on-error`, so an upload failure fails the job.
