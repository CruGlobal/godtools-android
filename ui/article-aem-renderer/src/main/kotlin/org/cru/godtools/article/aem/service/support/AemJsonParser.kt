...
fun JSONObject.findAemArticles(url: Uri): Sequence<Article> = when (optString(TAG_TYPE)) {
    ...
    TYPE_PAGE -> when {
        SUPPORTED_TEMPLATES.contains(optJSONObject(TAG_CONTENT)?.optString(TAG_TEMPLATE)) ->
            sequenceOf(parseAemArticle(url))
        else -> parseNestedAemObject(url)
    }
 ...
 */
private fun JSONObject.parseNestedAemObject(url: Uri) = keys().asSequence()
    .filterNot { isAemMetadataKey(it) }
    .flatMap { optJSONObject(it)?.findAemArticles(url.buildUpon().appendPath(it).build()).orEmpty() }

/**
 * Parse an AEM JSONObject into an Article
 */
private fun JSONObject.parseAemArticle(url: Uri) = Article(url).also { article ->
 ...
 */
private fun isAemMetadataKey(key: String) = key.startsWith("jcr:") || key.startsWith("cq:") || key.startsWith("sling:")
