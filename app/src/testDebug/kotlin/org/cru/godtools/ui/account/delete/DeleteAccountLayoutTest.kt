...
class DeleteAccountLayoutTest {
    ...
    fun `State Display - Action - Close Icon`() {
        composeTestRule.run {
            setContent { DeleteAccountLayout(UiState.Display(events)) }
    ...
    fun `State Display - Action - Delete Button`() {
        ...
        events.assertEvent(UiEvent.DeleteAccount)
    }
    ...
    fun `State Deleting - Action - Close Icon`() {
        composeTestRule.run {
            setContent { DeleteAccountLayout(UiState.Deleting(events)) }
            onNodeWithTag(TEST_TAG_ICON_CLOSE)
                .assertIsEnabled()
                .performClick()
        }
    ...
    fun `State Error - Action - Close Icon`() {
        composeTestRule.run {
            setContent { DeleteAccountLayout(UiState.Error(events)) }
            onNodeWithTag(TEST_TAG_ICON_CLOSE)
                .assertIsEnabled()
                .performClick()
        }
