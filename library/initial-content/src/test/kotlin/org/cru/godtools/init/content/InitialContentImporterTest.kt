...
class InitialContentImporterTest {
    private val legacyDataMigration = Job().apply { complete() }
    ...
    fun `Verify All Tasks run`() = runTest {
        InitialContentImporter(tasks, legacyDataMigration, UnconfinedTestDispatcher(testScheduler))
    ...
    @Test
    fun `All Tasks - Dependent on Legacy Data Migration completing`() = runTest {
        val legacyDataMigration = Job()
        InitialContentImporter(tasks, legacyDataMigration, UnconfinedTestDispatcher(testScheduler))
