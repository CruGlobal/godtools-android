...
class ManifestParser private constructor(private val context: Context) {
    ...
    fun parseManifest(manifestName: String, toolCode: String, locale: Locale) =
        synchronized(ThreadUtils.getLock(loadingLocks, manifestName)) {
            cache[manifestName] ?: runBlocking {
                try {
                    parse(manifestName, toolCode, locale)
                } catch (e: IOException) {
                    Result.Error()
                }
            }
    ...
    private suspend fun parse(manifestName: String, toolCode: String, locale: Locale): Result =
