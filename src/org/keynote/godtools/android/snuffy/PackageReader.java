...
//		color	= setColorAlphaVal(color, alpha);
		...
		if (bUrlMode || bPhoneMode || bEmailMode){
			...
			theContainer.addView(hr);
		   	...
		   	processButtonButton(elPage, root, currPage, elButton, Integer.valueOf(1000+iButton), false, theContainer);
			...
			else if (panel != null) { // not all buttons have panels (e.g. url buttons)
	...
	private void processTitle(Element elPage, Element root, SnuffyPage currPage, Element elTitle) {
        ...
        if (elSubHeading != null)	tvSubHeading = createTitleSubHeadingFromElement(elSubHeading, titleMode);
...
//        }        
        ...
        if (bHasPeekPanel) { //i.e. (tvSubTitle != null)
		...
		if (bPhoneMode || bEmailMode) {
	        button.setOnClickListener(new View.OnClickListener() {
				...
				public void onClick(View v) {
					...
					{
						e.printStackTrace();
						if (bPhoneMode) Toast.makeText(mContext, "Cannot launch dialler"  , Toast.LENGTH_SHORT).show();
						if (bEmailMode) Toast.makeText(mContext, "Cannot launch email app", Toast.LENGTH_SHORT).show();
					}				
				}
			});			
		}
