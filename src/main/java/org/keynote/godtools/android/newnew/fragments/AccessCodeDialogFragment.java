...
public class AccessCodeDialogFragment extends DialogFragment {
    ...
    public void onAttach(Activity activity) {
        ...
        mListener = (AccessCodeDialogListener) getParentFragment();
        progress = new ProgressDialog(this.getActivity());
        progress.setTitle("Checking code..");
    }
        ...
        if (positive) {
            ...
            if (accessCode.isEmpty()) {
                ...
                try {
                    ...
                    progress.show();
                    GodToolsApi.getInstance(getContext()).legacy.getAuthToken(accessCode)
                            .enqueue(new Callback<ResponseBody>() {
                        @Override
                        public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                            if (response.code() == HttpURLConnection.HTTP_NO_CONTENT) {
                                String key = response.headers().get(HttpHeaders.AUTHORIZATION);
                                SharedPreferences defaultSharedPreferences = PreferenceManager.getDefaultSharedPreferences(AccessCodeDialogFragment.this.getParentFragment().getActivity());
                                defaultSharedPreferences.edit().putBoolean("translator_preference_key", true).putString(AUTH_DRAFT, key).commit();
                                 progress.dismiss();
                                AccessCodeDialogFragment.this.dismiss();
